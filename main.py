...
def setup_gatekeeper(ec2_client, key_name, public_sg_id, private_sg_id, subnet_id, proxy_ip):
    ...
    EOF
        ...
        # Install the required Python packages
        sudo /home/ubuntu/myenv/bin/pip install fastapi uvicorn mysql-connector-python requests --break-system-packages
    ...
    '''
    try:
        logging.info("Starting Trusted Host setup...")
        ...
        '''
        logging.info("Starting Gatekeeper setup...")
        ...
        logging.info("Gatekeeper and Trusted Host setup complete.")
        return gatekeeper_instance_id, trusted_host_instance_id
    except Exception as e:
        logging.error(f"Error in setup_gatekeeper: {e}")
        raise
...
def benchmark_cluster(gatekeeper_ip):
    ...
    num_rows = 50
    logging.info(f"Sending 1000 ping-read requests to Gatekeeper at {gatekeeper_ip}")
    for _ in range(1000):
        ...
        for attempt in range(retries):
            try:
                ...
                if attempt < retries - 1:
                    ...
                    logging.error(f"ping-read failed after {retries} attempts - item_id={item_id}")
    
    logging.info(f"Sending 1000 write requests to Gatekeeper at {gatekeeper_ip}")
    for _ in range(1000):
...
def wait_for_api(ip, port=8000, retries=10, delay=30):
    import time
    import requests
    ...
    for _ in range(retries):
        try:
            ...
            if response.status_code == 200:
                print(f"API at {url} is ready.")
                return True
        except requests.exceptions.RequestException:
            time.sleep(delay)
    print(f"API at {url} failed to respond after {retries * delay} seconds.")
    return False
...
def main():
    ...
    time.sleep(120)
    gatekeeper_ip = get_public_ip(gatekeeper_instance_id)
    if not wait_for_api(gatekeeper_ip):
        raise Exception(f"Gatekeeper API at {gatekeeper_ip} is not ready.")
    
    logging.info(f"Starting benchmarking with Gatekeeper IP: {gatekeeper_ip}")
